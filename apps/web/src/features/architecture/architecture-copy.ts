...
export const buildingBlocks: BuildingBlock[] = [
  {
    ...
    state:
      'None of their own. A Worker answers a request and forgets it. Anything that needs to be remembered lives in one of the pieces below.',
  ...
  },
  {
    ...
    state:
      'Built in. Each object gets its own little SQLite database that sticks around between requests and restarts. No outside cache or lock needed to stay consistent.',
    garden:
      'Every agent, every chat, and every task run is its own Durable Object, so they cannot collide.',
  },
  {
    id: 'workflows',
    name: 'Workflows',
    ...
    state:
      'The progress is the state. It is saved between steps and survives failure on its own.',
    garden:
      'Each task run and automation run is a Workflow. The workflow’s id is the run id, so starting the same run twice is harmless.',
  },
  {
    id: 'think',
    name: 'Think',
    ...
    state:
      'Keeps the conversation (messages, cached prompt, tool wiring) in its Durable Object’s storage. We call a child Think instance a “facet”: one per chat or run.',
    garden:
      'A chat, a task run, and an automation run are each a Think facet under one parent agent.',
  },
  {
    id: 'codemode',
    name: 'codemode',
    ...
    state:
      'Thrown away after it runs. It reaches tools through an API we hand it, not a shared disk.',
  ...
  },
  {
    ...
    state:
      'Has its own files under /workspace, separate from the codemode world.',
  ...
  },
  {
    ...
    state:
      'Each connection is a session. We run those sessions in a dedicated proxy.',
    garden:
      'Connectors register as MCP servers and the proxy brokers the calls.',
  },
  {
    id: 'postgres',
    name: 'Postgres',
    ...
    state:
      'The lasting record of anything you can list or search: users, agents, chats, runs, permissions.',
    garden:
      'Every list and permission check reads Postgres, never an agent’s in-memory state.',
  },
  {
    id: 'r2',
    name: 'R2',
...
export const planes: Plane[] = [
  {
    ...
    runtime: 'Postgres',
    blurb:
      'Everything you can list, open, or check permissions on lives in a normal database. The screen reads from here. It is the part that never forgets.',
    nodes: [
      'Users & workspaces',
      'Agents',
      'Chats',
  ...
  },
  {
    ...
    blurb:
      'The live, thinking part. Each agent is one always-addressable instance you reach by name. Every chat or task it works on runs in its own isolated child, so two conversations never trip over each other.',
  ...
  },
  {
    ...
    blurb:
      'Where things actually get done. Long jobs run as crash-proof workflows, code runs in a sandbox, files go to storage, and outside apps are reached through connectors.',
    nodes: [
      'Workflow',
      'code execution',
      'Linux container',
  ...
  facets: [
    {
      id: 'chat',
      name: 'A chat',
      key: 'one per conversation',
      sub: 'A single back-and-forth with its own history, files, and tools.',
    },
    {
      id: 'task',
      name: 'A task run',
      key: 'one per task',
      sub: 'A unit of assigned work, with the tools to do it and a record of what it produced.',
    },
    {
      id: 'automation',
      name: 'An automation run',
      key: 'one per run',
      sub: 'A scheduled job that runs on its own and reports back when done.',
    },
  ],
...
export const workflowsVision = {
  ...
  status:
    'Some of this exists today (durable runs, the event ledger, the sandbox). The progress-and-next-intent contract and fully agent-driven traversal are the direction, not the current state. Calling that out so nobody reads this as already shipped.',
}
...
export const flows: Flow[] = [
  {
    ...
    label: 'A task run',
    summary:
      'Assigned work that needs to finish even if a server dies halfway.',
    steps: [
      {
        actor: 'Server',
        title: 'A run is created',
        detail: 'A row goes into the database marking the task as running.',
      },
      {
        actor: 'Agent',
        title: 'The agent is told to start',
        detail: 'The server calls the agent that owns the task.',
      },
      {
        actor: 'Workflow',
        title: 'A workflow begins',
        detail:
          'From here the run is crash-proof and can resume or be cancelled.',
      },
      {
        actor: 'Workflow',
        title: 'It drives each turn',
        detail:
          'The workflow asks the agent to take the next turn, over and over.',
      },
      {
        actor: 'Child',
        title: 'The agent works',
        detail:
          'The task’s child instance thinks, calls tools, and saves what it makes.',
      },
      {
        actor: 'Workflow',
        title: 'It finishes',
        detail:
          'When the turn reports done, the run is closed. Retries and waiting were the workflow’s job.',
      },
    ],
  },
  {
    ...
    steps: [
      {
        actor: 'Schedule',
        title: 'A timer fires',
        detail: 'A small scheduler object wakes up on its own clock.',
      },
      {
        actor: 'Server',
        title: 'A run is created',
        detail: 'A row goes into the database for this automation run.',
      },
      {
        actor: 'Agent',
        title: 'The agent is told to start',
        detail: 'Same handoff as a task run.',
      },
      {
        actor: 'Workflow',
        title: 'A workflow runs the turn',
        detail: 'The crash-proof boundary is identical to a task.',
      },
      {
        actor: 'Child',
        title: 'The agent works and reports',
        detail: 'It runs against the automation’s instructions and finishes.',
      },
    ],
  },
  {
    ...
    label: 'A live chat',
    summary:
      'You are sitting there waiting, so this one streams instead of running as a workflow.',
    steps: [
      {
        actor: 'Browser',
        title: 'You open the chat',
        detail:
          'The browser connects to the agent and asks for this chat’s child.',
      },
      {
        actor: 'Agent',
        title: 'It checks access',
        detail:
          'The agent confirms you are allowed before handing over the child.',
      },
      {
        actor: 'Child',
        title: 'The prompt is assembled',
        detail:
          'Base instructions, the agent’s own, the workspace, and your skills get stitched together.',
      },
      {
        actor: 'Child',
        title: 'The reply streams',
        detail: 'Words stream back as the model writes, and tools run inline.',
      },
      {
        actor: 'Database',
        title: 'The basics get saved',
        detail:
          'Title and last message go to the database; the live conversation stays in the child.',
      },
    ],
  },
]
...
export const execution = {
  intro:
    'There are two ways for an agent to run code, and they are kept separate on purpose.',
  lanes: [
    {
      id: 'codemode',
      name: 'codemode',
  ...
  trust: [
    {
      id: 'auto',
      name: 'auto',
      body: 'Go ahead without asking. For safe, high-volume things you trust.',
    },
    {
      id: 'allow',
      name: 'allow',
      body: 'Fine to read on its own, but check before it writes or sends.',
    },
    {
      id: 'ask',
      name: 'ask',
      body: 'Ask first, every time. The request shows up in your inbox.',
    },
  ],
  ...
  layers: [
    {
      id: 'foundation',
      name: 'Base',
      body: 'The ground rules every Garden agent shares.',
    },
    {
      id: 'agent',
      name: 'This agent',
      body: 'Its name, role, and personal instructions.',
    },
    {
      id: 'workspace',
      name: 'Workspace',
      body: 'The company’s name and context.',
    },
    {
      id: 'skills',
      name: 'Skills',
      body: 'Whichever reusable skills are turned on, loaded from storage.',
    },
  ],
}
  ...
  gets: [
    {
      id: 'providers',
      name: 'Any model',
      body: 'Anthropic, OpenAI, and others behind one interface. Switching models is a config change, not a rewrite.',
    },
    {
      id: 'tools',
      name: 'Tool calling',
      body: 'Typed tools, structured output, and the call loop are handled for us.',
    },
    {
      id: 'streaming',
      name: 'Streaming',
      body: 'Words stream from the model all the way to the browser.',
    },
    {
      id: 'mcp',
      name: 'Tools & code',
      body: 'Connector tooling and code execution come with the runtime.',
    },
  ],
  evidence:
    'In the code: Think depends on the AI SDK (ai v6) plus the Anthropic, OpenAI, and React adapters.',
}
  ...
  rows: [
    {
      ...
      garden: 'On servers, reachable by name',
      bare: 'On your laptop, with your login',
    },
    {
      capability: 'Memory',
      garden: 'A database plus per-agent storage',
      bare: 'Nothing saved between sessions',
    },
    {
      capability: 'After a crash',
      garden: 'Resumes from the last step',
      bare: 'Gone with the process',
    },
    {
      capability: 'Many users',
      garden: 'One isolated instance per agent and run',
      bare: 'One process, no separation',
    },
    {
      capability: 'Permissions',
      garden: 'Per-tool levels, asks when it should',
      bare: 'No permission system',
    },
    {
      capability: 'Running code',
      garden: 'Managed sandbox and container',
      bare: 'Runs on your box; you secure it',
    },
    {
      capability: 'Shipping it',
      garden: 'One deploy, scales to zero',
      bare: 'No deployment story',
    },
  ],
  close:
