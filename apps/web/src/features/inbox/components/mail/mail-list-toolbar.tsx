...
export function MailListToolbar({
  ...
  return (
    <div onKeyDown={handleKeyDown} className="shrink-0 border-b p-3">
          ...
          <>
            ...
            {onOpenFilters ? (
              <Button
                ...
                className={cn(filterSummary && 'bg-primary/5 text-foreground')}
                aria-label={compact ? 'Filter mail' : undefined}
              >
                <Filter />
                {!compact && (filterSummary || 'Filter')}
              </Button>
            ) : null}
            <Label className="flex shrink-0 items-center gap-1.5 text-xs font-normal text-muted-foreground">
              {!compact ? 'Unread' : null}
              <Switch
                size="sm"
                checked={unreadOnly}
                onCheckedChange={onUnreadOnlyChange}
                aria-label={compact ? 'Unread only' : undefined}
              />
            </Label>
          </>
        )}
