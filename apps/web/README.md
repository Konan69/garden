...
Authenticate Wrangler with `pnpm --filter @garden/web exec wrangler login`.
...
Workers AI runs remotely. The direct web `dev` command is the advanced
remote-infrastructure mode: copy
...
- Sandbox containers are enabled by `pnpm dev:remote` or the direct web `dev`
  command via `wrangler.containers.jsonc`; normal `pnpm dev` skips containers.
