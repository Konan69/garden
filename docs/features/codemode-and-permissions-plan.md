...
| Connector permission decision/audit                | `packages/agent-runtime/src/runtime-mcp-controller.ts`, `packages/db/src/schema/audit.ts`                |
...
Code evidence: `packages/db/src/schema/capabilities.ts`, `connectors/capabilities.ts`, `packages/agent-runtime/src/runtime-mcp-controller.ts`.
...
The product goal remains: approval behavior should not depend on whether a tool call came directly from the model, from codemode, or from Executor MCP. The code is not fully unified yet.

Target shape:

1. classify every callable local/MCP tool with a risk class;
2. use one shared policy decision module for runtime, Executor dispatch, UI inspection, and tests;
3. keep enforcement at the Executor dispatch boundary because it owns upstream tokens;
...
| Runtime policy and Executor dispatch are not one shared policy module.               | Executor has the authoritative token boundary; runtime should avoid drifting logic.                                                                |
...
- Keep connector token injection inside Executor; agent DOs should not see upstream tokens.
