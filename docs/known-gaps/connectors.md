...
| Exa search should be a built-in agent tool, not an MCP connector — web search is a first-class agent capability, not an integration; routing it through the connector system is wrong abstraction                                                         | `packages/connectors/src/exa-search/`                                                                                                                                   | Medium   |

## Done

| Item                                                        | Evidence                                                                                                                                                                                                       |
| ----------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 5 connector manifests                                       | `packages/connectors/src/github/`, `packages/connectors/src/gmail/`, `packages/connectors/src/google-drive/`, `packages/connectors/src/slack/`, `packages/connectors/src/exa-search/` each with `connector.ts` |
| Direct Executor engine + MCP session Durable Objects        | `apps/web/src/lib/server/executor-runtime.ts`, `apps/web/src/lib/server/executor-engine/mcp.ts`                                                                                                                |
| OAuth connect flow through Executor                         | `apps/web/src/routes/api/executor/oauth/start.ts`, `apps/web/src/routes/api/oauth/callback.ts`                                                                                                                 |
...
| Connector registry                                          | `packages/connectors/src/registry.ts` — 5 connectors with OAuth/API-key support                                                                                                                                |
...
- Agents SDK owns RPC MCP response routing for overlapping calls (`cloudflare/agents#1558`, shipped in `agents@0.14.5`), so Garden does not add another request queue around Executor MCP sessions.
